    ...
    <div className="p-4 max-w-lg mx-auto font-sans">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {messages.map((msg) => (
            <div key={msg.id} className="p-3 bg-white border rounded-lg shadow-sm">
              <div className="flex justify-between items-start mb-2">
                <span className="text-xs font-semibold px-2 py-0.5 rounded bg-blue-100 text-blue-800">
                  {new Date(msg.scheduledAt).toLocaleString()}
                </span>
                <span className={`text-xs px-2 py-0.5 rounded ${msg.status === 'SENT' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                  {msg.status}
                </span>
              </div>
              <p className="text-sm text-gray-800 mb-2 whitespace-pre-wrap">{msg.content}</p>
              <div className="flex justify-end gap-2">
                <button

(function () {
  ...
  const BASE_URL = 'https://wrapper2.s4r41va.com'; // Ajuste conforme necessário
  ...
  async function openPanel(page, isFullScreen = false) {
    ...
    if (headerTitle) {
      if (page === 'campaign') {
        headerTitle.textContent = 'Envio em Massa';
      } else if (page === 'ai-settings') {
        headerTitle.textContent = 'Agente de Inteligência Artificial';
      } else {
        headerTitle.textContent = 'Agendamento';
      }
    }
  ...
  function addSidebarItem(label, icon, onClickId, page) {
    ...
    item.onclick = (e) => {
      e.preventDefault();
      // Pass isFullScreen true if page is 'campaign' or 'ai-settings'
      openPanel(page, page === 'campaign' || page === 'ai-settings');
    };
  ...
  function initSidebar() {
    ...
    addSidebarItem('Agente de IA', iconAi, 'saas-ai-btn', 'ai-settings');
